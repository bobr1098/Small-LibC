...
void __divdi3_test()
{
    ...
    else printf(GREEN("[PASS]") " pos/pos 64bit division\n");

    if (div(-68719476736, 2) != -34359738368)
	    printf(RED("[FAIL]") " neg/pos 64bit division\n");
    else printf(GREEN("[PASS]") " neg/pos 64bit division\n");

    if (div(68719476736, -2) != -34359738368)
	    printf(RED("[FAIL]") " pos/neg 64bit division\n");
    else printf(GREEN("[PASS]") " pos/neg 64bit division\n");

    if (div(-68719476736, -2) != 34359738368)
	    printf(RED("[FAIL]") " neg/neg 64bit division\n");
    else printf(GREEN("[PASS]") " neg/neg 64bit division\n");

    if (div(68719476736, 1) != 68719476736)
	    printf(RED("[FAIL]") " pos/1 64bit division\n");
    else printf(GREEN("[PASS]") " pos/1 64bit division\n");
    ...
    else printf(GREEN("[PASS]") " LLONG_MAX/1 64bit division\n");

    if (div(LLONG_MAX, LLONG_MAX) != 1)
	    printf(RED("[FAIL]") " LLONG_MAX/LLONG_MAX 64bit division\n");
    else printf(GREEN("[PASS]") " LLONG_MAX/LLONG_MAX 64bit division\n");

    if (div(7, 3) != 2 || div(-7, 3) != -2 || div(7, -3) != -2)
	    printf(RED("[FAIL]") " 64bit division rounding to zero\n");
    else printf(GREEN("[PASS]") " 64bit division rounding to zero\n");
}
