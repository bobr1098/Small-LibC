...
typedef enum {
    I_MATCH, I_CHAR, I_ANY, I_BOL, I_EOL, I_BRACKET, I_BACKREF,
    I_SPLIT, I_JMP, I_SAVE
} Opcode;
...
static int new_inst(ExecCtx *ex, Opcode op, int v1, int v2, int out1, int out2) {
    ...
    if (ex->nfa_len >= ex->nfa_cap) {
        ...
        if (!p) { ex->err = REG_ESPACE; return -1; }
        ex->nfa = p;
        ex->nfa_cap = n;
    }
    int id = ex->nfa_len++;
...
static int build_nfa(ExecCtx *ex, int node, int out) {
    ...
    switch (n->op) {
        case AST_EMPTY:      return out;
        case AST_CHAR:       return new_inst(ex, I_CHAR, n->v1, 0, out, -1);
        case AST_ANY:        return new_inst(ex, I_ANY, 0, 0, out, -1);
        case AST_BOL:        return new_inst(ex, I_BOL, 0, 0, out, -1);
        case AST_EOL:        return new_inst(ex, I_EOL, 0, 0, out, -1);
        case AST_BRACKET:    return new_inst(ex, I_BRACKET, n->v1, n->v2, out, -1);
        case AST_BACKREF:    return new_inst(ex, I_BACKREF, n->v1, 0, out, -1);
        case AST_CONCAT: {
...
static int eval_bracket(ExecCtx *ex, int start, int end, char c) {
    int invert = 0;
    int p = start + 1;
    if (ex->pat[p] == '^') { invert = 1; p++; }
    
    if (invert && c == '\n' && (ex->cflags & REG_NEWLINE)) return 0;
    ...
    while (p < end) {
        if (ex->pat[p] == '[' && (ex->pat[p+1] == ':' || ex->pat[p+1] == '.' || ex->pat[p+1] == '=')) {
            ...
            p += 2;
            int cstart = p;
            while (ex->pat[p] != type || ex->pat[p+1] != ']') p++;
            size_t clen = p - cstart;
            char buf[256] = {0};
            for (size_t i = 0; i < clen && i < 255; i++) buf[i] = ex->pat[cstart + i];
            
            if (type == ':') {
                if (strcmp(buf, "alnum") == 0 && isalnum((unsigned char)c)) match = 1;
                else if (strcmp(buf, "alpha") == 0 && isalpha((unsigned char)c)) match = 1;
                else if (strcmp(buf, "blank") == 0 && (c == ' ' || c == '\t')) match = 1;
                else if (strcmp(buf, "cntrl") == 0 && iscntrl((unsigned char)c)) match = 1;
                else if (strcmp(buf, "digit") == 0 && isdigit((unsigned char)c)) match = 1;
                else if (strcmp(buf, "graph") == 0 && isgraph((unsigned char)c)) match = 1;
                else if (strcmp(buf, "lower") == 0 && islower((unsigned char)c)) match = 1;
                else if (strcmp(buf, "print") == 0 && isprint((unsigned char)c)) match = 1;
                else if (strcmp(buf, "punct") == 0 && ispunct((unsigned char)c)) match = 1;
                else if (strcmp(buf, "space") == 0 && isspace((unsigned char)c)) match = 1;
                else if (strcmp(buf, "upper") == 0 && isupper((unsigned char)c)) match = 1;
                else if (strcmp(buf, "xdigit") == 0 && isxdigit((unsigned char)c)) match = 1;
            } else if (type == '=') {
                char s_c[2] = {c, 0};
                if (strcoll(buf, s_c) == 0) match = 1;
            } else if (type == '.') {
                if (clen == 1 && buf[0] == c) match = 1;
            }
            p += 2;
            last = -1;
            ...
            if (strcoll(s_start, s_c) <= 0 && strcoll(s_c, s_end) <= 0) match = 1;
            p += 2;
            last = -1;
        } else {
            char check_c = c;
...
static void add_thread(ExecCtx *ex, ThreadList *l, int pc, regoff_t *saves, int bref_id, int bref_offset, size_t sp, int step_id) {
    ...
    switch (inst->op) {
        ...
        case I_BOL:
            if (sp == 0) {
                if (!(ex->eflags & REG_NOTBOL)) add_thread(ex, l, inst->out1, saves, bref_id, bref_offset, sp, step_id);
        ...
        case I_EOL:
            if (sp == ex->len) {
                if (!(ex->eflags & REG_NOTEOL)) add_thread(ex, l, inst->out1, saves, bref_id, bref_offset, sp, step_id);
...
static int cleanup(ExecCtx *ex, ThreadList *clist, ThreadList *nlist, regoff_t *init_saves, regmatch_t *best_m, int ret) {
    if (ex->nfa) free(ex->nfa);
    if (ex->handled) free(ex->handled);
    if (clist->t) free(clist->t);
    if (clist->saves_pool) free(clist->saves_pool);
    if (nlist->t) free(nlist->t);
    if (nlist->saves_pool) free(nlist->saves_pool);
    if (init_saves) free(init_saves);
    if (best_m) free(best_m);
    return ret;
}

int regexec(const regex_t *preg, const char *string, size_t nmatch, regmatch_t pmatch[], int eflags) {
    if (!preg || !string || !preg->re_comp) return REG_BADPAT;
    
    CompData *cd = (CompData *)preg->re_comp;
    
    size_t slen = 0;
    while (string[slen]) slen++;
    ...
    if (ex.err) return cleanup(&ex, NULL, NULL, NULL, NULL, ex.err);

    ex.handled = (int *)calloc(ex.nfa_len, sizeof(int));
    if (!ex.handled) return cleanup(&ex, NULL, NULL, NULL, NULL, REG_ESPACE);
    ...
    for (size_t i = 0; i < ex.internal_nmatch * 2; i++) init_saves[i] = -1;
    ...
    for (size_t start = 0; start <= slen; start++) {
        ...
        for (size_t sp = start; sp <= slen; sp++) {
            ...
            if (ex.err) return cleanup(&ex, &clist, &nlist, init_saves, best_m, ex.err);
        ...
        if (ex.best_len != -1) {
            if (!(preg->re_cflags & REG_NOSUB) && nmatch > 0 && pmatch) {
                ...
                for (size_t i = 0; i < copy_n; i++) pmatch[i] = best_m[i];
